...
const getConfigFromProject = () => {
  ...
  let moduleNamePattern = path.join(cwd, '.chaika_cache', 'chaika', 'qunar_*');
  ...
  moduleDirs.forEach(moduleDir => {
    ...
    let routes = appJsonData['pages'].map(pagePath => {
      let reg = /^\.\/pages\//; //是否以 ./pages开头
      pagePath = reg.test(pagePath) ? pagePath : './' + pagePath;
      return pagePath;
    });

    moduleName = pkg['module'] || pkg['name'];
    nameSpaceRoutes[moduleName] = Array.from(new Set(routes));
...
const injectPageRoute = nameSpaceRoutes => {
  ...
  needUpdate(appJsDist, code, () => {
    ...
    fs.writeFile(appJsDist, code, err => {
      ...
      success(
        'Merge page routes into ' + path.relative(cwd, appJsDist) + ' Success!'
      );
    });
  });
};
...
let mergePkg = (nameSpaceAlias, nameSpaceProjectPkg) => {
  ...
  let mainJsonDist = path.join(DEST_DIR.split('/')[0], 'package.json');
