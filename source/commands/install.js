...
module.exports = {
    msg: {
        ...
        options: [
            {
                pattern: '-p, --platform [type]',
                desc: '根据平台批量安装对面的业务模块'
            }
        ]
    },
    process: (projectDir, arg, options) => {
        const platform = options && options.platform ? options.platform + '_modules' : 'modules';

        Object.keys(pkgJsonConst).some(key => {
            if (!pkgJsonConst[key]) {
                util.outLog.error(`Please configure '${key}' with package.json`);
                process.exit(1);
            }
        });
        ...
        if (params && params.indexOf('--')) {
            ...
            let modules = (require(path.join(projectDir, 'package.json')) || {})[platform];

            if (!modules) {
                util.outLog.fail(platform + ' 未设置');
                return;
            }
        ...
        function clearLib(moduleName) {
            if (fs.existsSync(libsDir)) {
                fs.readdirSync(libsDir).forEach(file => {
                    if (path.extname(file) == '.w' && file.split('-')[0] == moduleName) {
        ...
        function reWritePackageMoudle(moduleName, version) {
            let pkgConfig = require(path.join(projectDir, 'package.json')) || {},
                modules = pkgConfig[platform] || {};
            modules[moduleName] = version;
            pkgConfig[platform] = modules;
            fs.writeFileSync(path.join(projectDir, 'package.json'), JSON.stringify(pkgConfig, {}, 2), 'UTF-8');
        }

        function installModuleByGitBranch(name, gitBranch, callback) {
            let pName = name.indexOf(config.MAIN_MODULE_PREFIX) ? 'module_' + name : name,
                branch = gitBranch.substring(1);

            let cloneSh = shelljs.exec(`git clone ${moduleGitUrl.replace('{module}', pName)}`, {
                cwd: libsDir,
                silent: true
            });
            ...
            if (cloneSh.code == 0) {
                let pPath = path.join(libsDir, PREFIX + pName),
                    branchSh = shelljs.exec(`git branch -r > /dev/null && git checkout ${branch} > /dev/null`, {
                        cwd: pPath,
                        silent: true
                    });
                if (branchSh.code) {
                    util.outLog.warn(`Check Out Branch '${branch}' Error, Use 'master' branch.`);
                }
                clearLib(name);

                if (name.indexOf(config.MAIN_MODULE_PREFIX) < 0) {
                    // nanachi 项目，拷贝package.json 到 source 目录，再压缩
                    shelljs.cp(path.join(pPath, 'package.json'), path.join(pPath, config.SOURCE_DIR));
                }

                targz.compress(
                    {
                        src: path.join(pPath, name.indexOf(config.MAIN_MODULE_PREFIX) >= 0 ? '' : config.SOURCE_DIR),
                    ...
                    err => {
                        ...
                        if (err) {
                            ...
                            util.outLog.success(`Install '${name}@Git${gitBranch}' Success！`);
                        }
                        callback();
                    }
                );
        ...
        function installModuleByOnlineUrl(name, version, url, isInit, callback) {
            clearLib(name);
            if (version == '0.0.0') {
            ...
            const options = { url, encoding: null },
                handleSuccess = function handleSuccess(body) {
                    ...
                    fs.writeFileSync(path.join(libsDir, `${name}-${version}.w`), body);
                    if (version && !isInit) {
                        reWritePackageMoudle(name, version);
                    }
                    util.outLog.success(`Install '${name}@${version}' Success！`);
                },
        ...
        function installModuleByTag(name, tag, callback) {
            ...
            const url = `${pkgJsonConst.packageUrl}nnc_module_${name}/${tag}/${name}-${tag}.w`,
                handleSuccess = function handleSuccess(body) {
                    fs.writeFileSync(path.join(libsDir, `${name}-${tag}.w`), body);
                    ...
                    .then(
                        function handleSuccess(body) {
                            ...
                            if (versions.indexOf(version) > -1) {
                                let url = list.filter(item => item.version == version)[0].path;
                                installModuleByOnlineUrl(name, version, url, isInit, callback);
                            } else {
                                util.outLog.fail(`Get Module '${name}' Version List Fail!`);
                                callback();
                            }
                        },
