...
module.exports = {
  ...
  process: projectDir => {
    ...
    if (params && params.indexOf('--')) {
      ...
      let modules =
        (require(path.join(projectDir, 'package.json')) || {}).modules || {};
      async.eachSeries(
        Object.keys(modules),
        (key, callback) => {
    ...
    function installModuleByTag(name, tag, callback) {
      clearLib(name);
      const url = `${pkgJsonConst.packageUrl}${tag}/${name}-${tag}.w`,
        handleSuccess = function handleSuccess(body) {
    ...
    function installPackage(name, version = '', isInit, callback) {
      ...
      } else if (version && /^[br]-/.test(version)) {
        //nanachi 暂不支持btag rtag下载。
        return;
      } else {
          ...
          .then(
            function handleSuccess(body) {
              ...
              if (versions.indexOf(version) > -1) {
                ...
                util.outLog.fail(
                  `Get Module '${name}' Version < ${version} > Fail！`
                );
                callback();
              }
            },
